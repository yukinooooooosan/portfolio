...
- 公開URLは `https://portfolio-dwx.pages.dev`。
- 出力ディレクトリは `public`。
- ビルドコマンドなしで配信する構成。
...
- Cloudflare Pagesでは `portfolio-dwx.pages.dev` の下に公開される。
- 独自ドメインはまだ使っていない。
...
- 現在はGitHub PagesとCloudflare Pagesの標準ドメインを使っているため、独自DNS設定は不要。
- 将来独自ドメインを使う場合は、Cloudflare DNSなどで向き先を設定する可能性がある。
